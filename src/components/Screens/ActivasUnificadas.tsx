...
type ItemUnificado = {
  id: string; // prefijo + id, para evitar choques entre ambos tipos
...
export default function ActivasUnificadas({ navigation }: { navigation: ActivasUnificadasNavigationProp }) {
  ...
  const cargarActivas = async () => {
    try {
      ...
      console.log('Error al cargar asignaciones activas:', error);
    } finally {
      setCargando(false);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.titulo}>Asignaciones Activas</Text>
        <Text style={styles.subtitulo}>{items.length} elemento(s) prestado(s) actualmente</Text>

        {items.length === 0 ? (
          <Text style={styles.vacioTexto}>No hay nada prestado actualmente.</Text>
        ) : (
          items.map((item) => (
            <View key={item.id} style={styles.itemCard}>
              ...
              <View style={styles.itemInfo}>
                ...
                <Text style={styles.itemFecha}>
                  Desde: {new Date(item.fechaAsignacion).toLocaleDateString()}
                </Text>
              </View>
              ...
              <View
                style={[
                  styles.tipoBadge,
                  { backgroundColor: item.tipo === 'herramienta' ? '#e3f2fd' : '#f3e5f5' },
                ]}
              >
                <Text
                  style={[
                    styles.tipoTexto,
                    { color: item.tipo === 'herramienta' ? '#2196F3' : '#9C27B0' },
                  ]}
                >
                  {item.tipo === 'herramienta' ? 'Herramienta' : 'Carro'}
                </Text>
              </View>
            </View>
          ))
...
const styles = StyleSheet.create({
  ...
  itemFecha: { fontSize: 11, color: '#999', marginTop: 4 },
  tipoBadge: {
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 8,
  },
  tipoTexto: { fontSize: 10, fontWeight: 'bold' },
});